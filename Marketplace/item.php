		...
		if($account > 0){
			...
			if(isset($_GET['id'])){
				...
				if(mysqli_num_rows($hatExists) > 0){
					...
					// global vars
					if($id < 1){
						header("Location: ../Marketplace/"); exit;
					}
					...
					if(isset($_GET['remove']) && is_numeric($_GET['remove'])){
						...
						if(mysqli_num_rows($ourSale) > 0){
							...
							header("Location: ../Marketplace/item.php?id=$Hat[0]"); exit;
						}else{
													...
													if($EnoughMoney >= 0){
														...
														if(mysqli_num_rows($haveItem) < 1){
															...
															if(mysqli_num_rows($alreadyGotQ) > 0){
																...
																echo"<script>window.location='../Marketplace/item?id=$id'</script>"; exit;
															}
									...
									echo"<br>
										...
										if(mysqli_num_rows($hasComments) > 0){
											...
											if(isset($_GET['page']) && mysqli_num_rows($getComments) < 1){
												echo"<script>window.location='../Marketplace/item.php?id=$id';</script>";
												exit;
											}
											...
											while($CommentData = mysqli_fetch_array($getComments)){
												...
												<div style='border:1px solid;width:900px;margin-bottom:10px;display:flex;'>
													...
													<div style='margin:0px;text-align:right;'>
														<a href='/Mod/reportcomment.php?comment=$CommentData[0]' style='text-align:left;font-size:12.5px;color:grey;width:100px;padding-left:5px;'>Report</a>
													</div>
														<br>
					...
					// bye!
					echo"<script>window.location='../Marketplace';</script>"; exit;
				}
			}else{
				# bye!
				echo"<script>window.location='../Marketplace';</script>"; exit;
