		...
		if($account > 0){
			...
			if(isset($_GET['comment'])){
				if(is_numeric($_GET['comment'])){
					...
					if($user[0] != $comment){
						// see if user is real
						$RealUser = mysqli_query($conn, "SELECT * FROM `ec_item_comments` WHERE `ID`='$comment'");
						$RealUser2 = mysqli_query($conn, "SELECT * FROM `ec_item_comments` WHERE `ID`='$comments'");
						if(mysqli_num_rows($RealUser) > 0){
							// get user info
							$uInfo = mysqli_fetch_array($RealUser);
							$uInfo2 = mysqli_fetch_array($RealUser2);
							...
							if(isset($_POST['reason'])){
								...
								if(strlen($reason) > 7){
									...
									echo"<script>window.location='../Marketplace/?id=$uInfo[0]'</script>"; exit;
								}else{
								echo"
									<center>
										<div style='height:115px;'></div> <!-- SPACE -->
											<div id='platform' style='width:1200px; border:1px solid black;background-color:white;border-radius:10px;padding:20px;'>
												<h1 style='text-align:left;padding-left:200px;'>Report comment number : $comment</h1>
												<h1 style='text-align:left;padding-left:200px;'>Comment : $comments</h1>
												<form method='post'>
