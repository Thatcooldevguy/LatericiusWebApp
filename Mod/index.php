		...
		if($account > 0){
			...
			if($user['POWER']!='ADMIN' && $user['POWER']!='MODERATOR'){
				...
				if(isset($_GET['approveAss']) && is_numeric($_GET['approveAss'])) {
					...
					// go to item
					//exit(header("Location: ../Marketplace/item.php?id=$theirs[ID]"));
					echo"<script>window.location='../Marketplace/item.php?id=$theirs[ID]'</script>"; exit;
				}
			}
				...
				<center>
					<div style='height:115px;'></div> <!-- SPACE -->
						<div id='platform' style='width:1200px; border:1px solid black;background-color:white;border-radius:10px;padding:20px;'>
							<h1 style='text-align:left;'>Moderation Panel (Layout and Beauty is minimal)</h1><a style='font-size:10px;color:grey;' href='gift'>Open Gifts?</a>";
