<?php
	...
	if(ISSET($_COOKIE['EPICNAME']) && ISSET($_COOKIE['EPICPASS'])){
		...
		if($account > 0){
			...
			if($user['POWER']!='CO-FOUNDER' || $user['USERNAME']!='b'){ // only founders, low sec checks :) 
			...
			if(isset($_POST['giftid'])){
				...
				mysqli_query($conn, "INSERT INTO `ec_crate` VALUES(NULL,'$item_id','$gift_id','0')");
				mysqli_query($conn, "INSERT INTO `ec_mod_logs` VALUES(NULL, '$user[0]','$user[0]','Gave #$gift_id a #$item_id','$curtime')");
				echo "Success! - Redirecting";
				sleep(1);
				echo"<script>window.location='../Marketplace/item?id=$item_id'</script>";	
				exit;
			}else{	
				echo"
					<center>
						<div style='height:115px;'></div> <!-- SPACE -->
							<div id='platform' style='width:1200px; border:1px solid black;background-color:white;border-radius:10px;padding:20px;'>
								<form method='post'>
								User ID <input name='giftid'><Br>
								Item ID <input name='itemid'><br> 
