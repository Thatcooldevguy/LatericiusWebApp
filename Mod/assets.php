	...
	if(ISSET($_COOKIE['EPICNAME']) && ISSET($_COOKIE['EPICPASS'])){
		...
		if($account > 0){
			...
			if(isset($_GET['d'])){
				...
				echo "<script>window.location='../Mod/assets.php'</script>"; exit;
			}
			...
			if(isset($_GET['a'])){
				...
				echo"<script>window.location='../Marketplace/?type=$asset[TYPE]'</script>"; exit;
			}
								...
								while($Asset = mysqli_fetch_array($AssetsQ)){
									...
									<div style='width:850px;border:1px solid grey;display:flex;flex-wrap:wrap;margin-bottom:20px;'>
										...
										<div style='padding-left:225px;padding-top:25px;'>
											<a href='assets.php?a=$Asset[ID]' style='width:50px;padding:2.5px;border:1px solid green;color:green;'>Accept</a>
											- 
											<a href='assets.php?d=$Asset[ID]' style='width:50px;padding:2.5px;border:1px solid red;color:red;'>Decline</a>
										</div>
									</div>
									";
								}
