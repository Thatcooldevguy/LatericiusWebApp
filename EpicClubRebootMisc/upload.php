			...
			if(isset($_POST['name']) && isset($_POST['description']) && isset($_FILES["file"]) && isset($_POST['price'])){
				...
				if ($_FILES["file"]["error"] > 0){
					...
					if ($info === FALSE) {
					  ...
					  echo"<script>window.location='../Marketplace/upload.php'</script>"; exit;
					}
					...
					if(is_numeric($price)){
						...
						if($user['VIP']=='NONE'){
							...
							if(mysqli_num_rows($numberOfU) > 24){
								echo"<script>alert('You have reach the limit for uploads (25) For more uploads please upgrade')</script>";
						...
						mysqli_query($conn, "INSERT INTO `ec_user_assets` VALUES(NULL,'$name','$description','$user[0]','$curtime','$price','$imageAVATAR_URL','PENDING','0','$type')");
